...
def test_visualization_plots(tmp_path):
    ...
    fig1 = plot_sensor_degradation_trends(
        ...
        save_path=str(tmp_path / "trends.html"),
    )
    assert fig1 is not None
    assert (tmp_path / "trends.html").exists()

    fig2 = plot_correlation_heatmap(
        df_sample, ["temperature", "pressure"], save_path=str(tmp_path / "corr.png")
    )
    ...
    fig4 = plot_model_comparison_bar(
        metrics_dict, metric_name="RMSE", save_path=str(tmp_path / "comp.html")
    )
